...
import threadpoolmonitor.optbuild.Options;
...
public class ThreadPoolMonitor {
                ...
                do {
                    ...
                    if (exited != null) {
                        for(WorkerThreadReport r : exited) {
                            System.out.printf("--worker #%02d exited after shutdown or %d s of inactivity%n",
    ...
    /**
     *  If QUEUE_SIZE is greater or equal the number os tasks, only core pool size
    ...
    private static  ThreadPoolExecutor createThreadPool(int coreSize, int maxSize, int queueSize) {
        return theThreadPool = new ThreadPoolExecutor(
                coreSize										/* int corePoolSize */,
                maxSize											/* int maximumPoolSize */,
                KEEP_ALIVE_SECONDS, TimeUnit.SECONDS			/* long keepAliveTime, TimeUnit unit */,
                new ArrayBlockingQueue<Runnable>(queueSize)		/* BlockingQueue<Runnable> workQueue */,
    ...
    public static void main(String[] args)   {
        ...
        createThreadPool(opt.minSize, opt.maxSize, opt.queueSize);
